...
const allTrans = (req, res)=>{
    ...
    if (userCookie){
        const sql = `
      SELECT * FROM bkew76jt01b1ylysxnzp.re_transaction ORDER BY id DESC;
    `;
...
const oneTrans = (req, res, next)=>{
    ...
    if (!userCookie) {
        ...
        const sql = `
      SELECT * FROM bkew76jt01b1ylysxnzp.re_transaction WHERE id =?;
    `;
...
const createTrans = (req, res, next) => {
    ...
    try {
        db.query('INSERT INTO bkew76jt01b1ylysxnzp.re_transaction SET ?', { title , description ,Trans_status, price, location  });
