...
const allComplain = (req, res) => {
    ...
    if (userCookie) {
        const sql = `
      SELECT * FROM bkew76jt01b1ylysxnzp.re_complaint ORDER BY id DESC;
    `;
...
const allMyComplain = (req, res) => {
    ...
    if (userCookie) {
        const sql = `
      SELECT * FROM bkew76jt01b1ylysxnzp.re_complaint WHERE user_id = ? ORDER BY id DESC;
    `;
...
const oneComplain = (req, res) => {
    ...
    if (!userCookie) {
        ...
        const sql = `
      SELECT * FROM bkew76jt01b1ylysxnzp.re_complaint WHERE id =?;
    `;
        ...
        try {
          ...
          `;
            ...
            db.query(sql, [id], (err, results) => {
                ...
                db.query('INSERT INTO bkew76jt01b1ylysxnzp.re_complaint SET ?', { title , user_id,  name, complain, number,  aacount_id, report_id });
