...
const allRept = (req, res) => {
    ...
    if (userCookie) {
        const sql = `
      SELECT * FROM bkew76jt01b1ylysxnzp.re__report ORDER BY id DESC;
    `;
...
const allMyRept = (req, res) => {
    ...
    if (userCookie) {
        const sql = `SELECT * FROM bkew76jt01b1ylysxnzp.re__report WHERE user_id = ? ORDER BY id DESC;`;
...
const oneRept = (req, res) => {
    ...
    if (!userCookie) {
        ...
        db.query('SELECT * FROM bkew76jt01b1ylysxnzp.re__report WHERE report_id =?', [report_id], (err, result) => {
            if (err) {
            ...
            const sql = `
      SELECT * FROM bkew76jt01b1ylysxnzp.re__content WHERE report_id =?;
    `;
...
const createRept = (req, res, next) => {
    ...
    try {
        db.query('INSERT INTO bkew76jt01b1ylysxnzp.re_report SET ?', { title, description, Rept_status, price, location });
...
const deleteRept = (req, res, next) => {
    ...
    if (userCookie) {
        ...
        try {
            ...
            const sql = `DELETE FROM bkew76jt01b1ylysxnzp.re__content WHERE report_id = ?;`;
            db.query(sql, [report_id], (err, result) => {
                ...
                const sql = `DELETE FROM bkew76jt01b1ylysxnzp.re__report WHERE report_id = ?;`;
                db.query(sql, [report_id], (err, result) => {
                    if (err) {
                        console.error('Error deleting report:', err);
                        return res.status(500).send('Internal Server Error');
